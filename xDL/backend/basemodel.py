...
class BaseModel(tf.keras.Model):
    ...
    def _get_model_specifications(self, batch_size, val_split, random_state):
        ...
        for idx, name in enumerate(FH.terms):
            if ":" in name:
                ...
                self.input_dict[name] = {
                    "Network": FH.feature_nets[idx],
                    "Sizes": FH.hidden_layer_sizes[idx],
                }
            else:
                self.input_dict[name] = {
                    "Network": FH.feature_nets[idx],
        ...
        if "Transformer" in named_feature_nets:
            output_mode = "int"
        else:
            output_mode = "one_hot"
        ...
        hidden_layer_sizes = FH.hidden_layer_sizes
    ...
    def _get_dataset(self, data, batch_size=512, shuffle=False, output_mode="one_hot"):
        """
