...
class doubleNAM(AdditiveBaseModel):
    """
    ...
    def __init__(
        ...
        self.dense_output_layer = tf.keras.layers.Dense(1, activation="linear")
    ...
    def call(self, inputs, training=False):
        ...
        for layer in self.fully_connected:
            x = layer(x)
        summed_outputs = self.output_layer(summed_outputs)
        output = self.dense_output_layer(x)
        return summed_outputs, output
    ...
    def _get_validation_preds(self):
        ...
        """
        preds = self.predict(self.validation_dataset)[1]
        return preds
